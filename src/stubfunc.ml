...
struct
    ...
    let stub addr_call vsa ha hf _number_init _addr _func_name _call_number _backtrack  =
        if (List.exists (fun x -> x = addr_call) addr_skip) then true, restore_esp vsa, ha, hf 
        else false,vsa,ha,hf
