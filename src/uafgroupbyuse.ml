...
module UafGroupByUse (Absenv_v : AbsEnvGenerique) =
struct
	...
	type site = Uafgenerique.site
        ...
        * key is chunk * free site * use site, because from a same malloc, different free that lead to different uaf 
        * *)
        let sg_uaf = ((Hashtbl.create 100) : (( (int*Absenv_v.chunk_t) * (((site*site_type) list) list) * (((site*site_type) list) list)  , ((site*site_type) list)  ) Hashtbl.t )) 
        ...
        let add_uaf ?(t=SITE_USE) c state =
            ...
            let key = Absenv_v.get_chunk_key c,c_free,state in
            try
                let alloc=Hashtbl.find sg_uaf key in
                Hashtbl.replace sg_uaf key (alloc)
            with
                Not_found -> Hashtbl.add sg_uaf key (c_alloc)
        ...
        let get_sg_uaf_by_alloc () =
                ...
                let () =
                    Hashtbl.iter 
                        (fun (((chunk_id,chunk_type),free,use)) (alloc) ->
